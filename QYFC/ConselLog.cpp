...
const int QYConsoleLog::ToDebug = 1;
const int QYConsoleLog::ToFile = 2;
const int QYConsoleLog::ToConsole = 4;

const static int LINE_BUFFER_SIZE = 1024;

QYConsoleLog::QYConsoleLog(int mode, int level, char *filename, bool append)
{
	m_lastLogTime = 0;
	m_filename = NULL;
	m_append = false;
...
void QYConsoleLog::SetMode(int mode) {
...
int QYConsoleLog::GetMode() {
	return m_mode;
}

void QYConsoleLog::SetLevel(int level) {
    m_level = level;
}

int QYConsoleLog::GetLevel() {
	return m_level;
}

void QYConsoleLog::SetStyle(int style) {
	m_style = style;
}

int QYConsoleLog::GetStyle() {
	return m_style;
}

void QYConsoleLog::SetFile(const char *filename, bool append)
{
	CloseFile();
	if (m_filename != NULL)
		free(m_filename);
...
inline void QYConsoleLog::ReallyPrintLine(char *line)
{
    if (m_todebug) 
		OutputDebugStringA(line);
    if (m_toconsole) {
        DWORD byteswritten;
        WriteConsoleA(GetStdHandle(STD_OUTPUT_HANDLE), line, strlen(line), &byteswritten, NULL);
    }
...
void QYConsoleLog::ReallyPrint(char *format, va_list ap)
{
